...
with open(os.path.join(prefix, '53-join.txt')) as f:
    ...
    for row in reader:
        ...
        if len(senses) > 2 and len(hypernyms) > 2:
            clusters[cid] = Cluster(cid, senses, hypernyms)
        else:
            logging.warn('Cluster %s has only %d sense(s) and %d hypernyms, skipping.', cid, len(senses), len(hypernyms))

def emit(cid):
    cluster = clusters[cid]

    hypernyms = list({w.lower() for w in cluster.hypernyms})
    random.shuffle(hypernyms)
    hypernyms = hypernyms[:HYPERNYMS]
    logging.info('Starting cluster %s: %s.', cid, ', '.join(hypernyms))

    try:
        mfw, _ = max(((w, freqs[w]) for w in hypernyms if w in freqs), key=operator.itemgetter(1))
    except ValueError:
    ...
    generator = ((freqs_keys[i], freqs[freqs_keys[i]])
                 ...
                 if freqs_keys[i].lower() not in hypernyms
    ...
    hypernyms.append(intruder)
    random.shuffle(hypernyms)
    intruder_index = hypernyms.index(intruder)

    row = [cid, hypernyms.index(intruder) + 1]
    for i in xrange(HYPERNYMS + 1):
        if len(hypernyms) > i:
            row.append(hypernyms[i])
        else:
            row.append(None)
    for i in xrange(SENSES):
        if len(cluster.senses) > i:
            row.append(cluster.senses[i])
        else:
            row.append(None)
    logging.info('Emitted a task for cluster %s with %d hypernyms.', cid, len(hypernyms))
    return row

with open(os.path.join(prefix, '62-hypernyms-hit.tsv'), 'wb') as f:

...
In order to evaluated against BabelNet, the `make` command should be ran with the `BABELNET_EVAL=1` flag. Note that such an evaluation requires the `babelnet-extract` dependency to be correctly provided with either an API key or an offline index. In this case, the following files can also be of interest:
